...
func main() {
	...
	flag.BoolVar(&cfg.HistogramProcessLabelsEnabled, "histogram-process-labels", false, "Emit process label on histogram metric families; default false drops it from histogram bucket/sum/count series to reduce cardinality while keeping process-labeled duration counters")
