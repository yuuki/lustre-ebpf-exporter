...
1. **llite** — user-facing workload plane: required kprobes on `ll_lookup_nd`, `ll_file_open`, `ll_file_read_iter`, `ll_file_write_iter`, `ll_fsync` plus optional kprobe+kretprobe pairs on `ll_file_release` (close), `ll_getattr`, `ll_xattr_get_common`/`ll_xattr_get` (getxattr), `ll_xattr_set_common`/`ll_xattr_set` (setxattr), `ll_mkdir`, `ll_mknod`, `ll_rename`, `ll_rmdir`, `ll_setattr`, `ll_statfs`, `ll_create_nd`, `ll_unlink`, `ll_link`, `ll_symlink`, `ll_listxattr`, `ll_get_acl`, `ll_get_inode_acl`, `ll_set_acl`, `ll_get_link`, `ll_iterate`. Each op is classified by `lustre.access.intent`: `namespace_read`, `namespace_mutation`, `data_read`, `data_write`, `sync`. The metadata kprobe group assumes Linux 5.12+ idmapped-mount inode_operation signatures for several probes (PARM2 holds inode/dentry/path), while compatibility probes cover some older Lustre 2.14-era symbols. On unsupported signatures the symbols may still attach but `s_dev` extraction may fail and emit nothing. All metadata probes are optional and degrade gracefully.
